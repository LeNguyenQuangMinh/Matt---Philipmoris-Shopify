...
class MenuDrawer extends HTMLElement {
  ...
  bindEvents() {
    ...
    this.querySelectorAll(
      "button:not(.localization-selector):not(.selector__close-button):not(.country-filter__reset-button):not(.js-brand-filter-btn)"
    ).forEach((button) =>
...
class SwiperComponent extends HTMLElement {
  constructor() {
    ...
    this.isTabletOnly = this.hasAttribute("data-swiper-tablet");
    this.swiperEl = null;
    this.initSwiper = null;
    this.options = null;
    this.breakpoint = null;
    this.breakpointTablet = null;
  ...
  disconnectedCallback() {
    ...
    if (this.breakpointTablet && this.breakpointTabletChecker) {
      this.breakpointTablet.removeEventListener("change", this.breakpointTabletChecker);
    }
    if (this.initSwiper) {
  ...
  initializeSwiper() {
    if (this.items.length > 0) {
      ...
      const nextButton = getOption("arrows-outside") == true ? document.querySelector(`[data-slide-arrows-id="${this.getAttribute("data-slide-id")}"].swiper-button-next`) : this.swiperEl.querySelector(".swiper-button-next");
      const prevButton = getOption("arrows-outside") == true ? document.querySelector(`[data-slide-arrows-id="${this.getAttribute("data-slide-id")}"].swiper-button-prev`) : this.swiperEl.querySelector(".swiper-button-prev");
  ...
  initSwiperTablet() {
    this.breakpointTablet = mqlDesktop;
    const enableSwiper = () => {
      if (!this.swiperEl || !this.options) return;
    ...
    if (this.isTabletOnly) {
      this.breakpointTablet.addEventListener("change", this.breakpointTabletChecker);
    }
