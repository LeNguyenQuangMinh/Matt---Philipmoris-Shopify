...
function setCartDrawerEmptyState(isEmpty, options = {}) {
  const { measureHeight = true } = options;
  ...
  if (!emptyBlock || !filledBlock) return;

  cartDrawer.classList.toggle('is-empty', isEmpty);
  emptyBlock.classList.toggle('hidden', !isEmpty);
  filledBlock.classList.toggle('hidden', isEmpty);
  cartDrawer.querySelector('cart-drawer-items')?.classList.toggle('is-empty', isEmpty);
...
function updateCartEmptyStateVisibility(itemCount) {
  if (itemCount === undefined || itemCount === null) return;
  ...
  if (document.querySelector('cart-drawer')) {
    setCartDrawerEmptyState(itemCount === 0);
  }
}

window.updateCartEmptyStateVisibility = updateCartEmptyStateVisibility;
window.setCartDrawerEmptyState = setCartDrawerEmptyState;

function syncCartPageExtrasFromSection(targetSection, newSection) {
  const newCountdown = newSection?.querySelector?.('.cart-page__message .cart-countdown');
  const targetCountdown = targetSection?.querySelector?.('.cart-page__message .cart-countdown');

  if (newCountdown && targetCountdown) {
    targetCountdown.classList.toggle('hidden', newCountdown.classList.contains('hidden'));
  }

  const newShipping = newSection?.querySelector?.('.cart-page__shipping');
  const targetShipping = targetSection?.querySelector?.('.cart-page__shipping');

  if (!newShipping && targetShipping) {
    targetShipping.classList.add('hidden');
  } else if (newShipping && targetShipping) {
    targetShipping.classList.toggle('hidden', newShipping.classList.contains('hidden'));
  } else if (newShipping && !targetShipping) {
    const insertBefore = targetSection.querySelector('.cart-page__message');
    if (insertBefore) {
      insertBefore.before(newShipping.cloneNode(true));
    }
  }
}
...
class CartItemsComponent extends HTMLElement {
  ...
  async onLineItemRemove(line) {
    ...
    if (willBeEmpty && this.closest('cart-drawer')) {
  ...
  #updateSingleSection(sectionId, html) {
    ...
    syncCartPageExtrasFromSection(target, newSection);
  }
  ...
  async updateQuantity(config) {
    ...
    try {
      ...
      if (json.sections) {
        this.updateSections(json.sections, json);
      }

      document.dispatchEvent(new CustomEvent('cart:updated', { detail: json }));
      publish(PUB_SUB_EVENTS.cartUpdate, { source: 'cart-items-component', cartData: json });
      ...
      if (json.item_count !== undefined) {
        ...
        updateCartEmptyStateVisibility(json.item_count);
      }
  ...
  handleCartUpdate(event) {
    ...
    const itemCount =
      event?.detail?.item_count ??
      event?.detail?.data?.item_count ??
      event?.detail?.cartData?.item_count;

    if (sections) {
      this.updateSections(sections, event?.detail);
    }

    updateCartEmptyStateVisibility(itemCount);
  };
...
class CartDrawerItems extends CartItemsComponent {
  ...
  updateSections(sections, cartData, options = {}) {
    if (!sections) return;
    ...
    if (itemCount === 0) {
      setCartDrawerEmptyState(true, { measureHeight });
      return;
    }

    setCartDrawerEmptyState(false, { measureHeight });

    const cartDrawerSectionKey = Object.keys(sections).find((key) => key.includes('cart-drawer'));
    const cartDrawerHtml = sections['cart-drawer'] || (cartDrawerSectionKey ? sections[cartDrawerSectionKey] : null);

    if (cartDrawerHtml) {
      this.#updateCartDrawerContentsFromHTML(cartDrawerHtml, { measureHeight });
      return;
    }
  ...
  #updateCartDrawerContentsFromHTML(html, options = {}) {
    const { measureHeight = true } = options;
    const newDOM = new DOMParser().parseFromString(html, 'text/html');
    const cartDrawer = document.querySelector('cart-drawer');
    if (!cartDrawer || !html?.trim()) return;

    const newPreviewMessage = newDOM.querySelector('.previewCartMessage');
    const targetPreviewMessage = cartDrawer.querySelector('.previewCartMessage');

    if (newPreviewMessage && targetPreviewMessage) {
      targetPreviewMessage.replaceWith(newPreviewMessage.cloneNode(true));
    } else if (targetPreviewMessage) {
      targetPreviewMessage.classList.remove('hidden');
    }

    const newCartCount = newDOM.querySelector('.wrapper-cart-count');
    const targetCartCount = cartDrawer.querySelector('.wrapper-cart-count');
    if (newCartCount && targetCartCount) {
      targetCartCount.innerHTML = newCartCount.innerHTML;
    }

    const newCartDrawerItems = newDOM.querySelector('cart-drawer-items');
    const target = cartDrawer.querySelector('cart-drawer-items');

    if (newCartDrawerItems && target) {
    ...
    const targetFooter = cartDrawer.querySelector('.cart-drawer__footer');
    if (newCartDrawerFooter && targetFooter) {
      targetFooter.innerHTML = newCartDrawerFooter.innerHTML;
    }

    const newPurchaseConditions = newDOM.querySelector('[data-purchase-conditions]');
    const targetPurchaseConditions = cartDrawer.querySelector('[data-purchase-conditions]');
    if (newPurchaseConditions && targetPurchaseConditions) {
      targetPurchaseConditions.innerHTML = newPurchaseConditions.innerHTML;
      if (typeof initPurchaseConditions === 'function') {
        delete targetPurchaseConditions.dataset.purchaseConditionsInit;
        initPurchaseConditions(targetPurchaseConditions);
      }
    }
